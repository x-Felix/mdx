...
// 此处修正原因：当到达下边的else if时，$mdx_flag_subdir 必定为true，因为!$mdx_flag_subdir 已经为false
if (!$mdx_flag_subdir) {
    global $wp;
...
//自定义菜单样式
function mdx_menu_classes($classes, $item, $args)//求问这个$item的作用是什么？
{
...
function mdx_css_login() {
    ...
    if (mdx_get_option("mdx_login_md") == "true") {
        wp_register_style('mdx_mdui_css_login', $files_root.'/mdui/css/mdui.min.css', '', '');
        wp_register_style('mdx_style_css_login', $files_root.'/css/login.css', '', '');
        wp_enqueue_style('mdx_mdui_css_login');
        wp_enqueue_style('mdx_style_css_login');
        if (mdx_get_option("mdx_md2") == "true") {
            wp_register_style('mdx_md2_login', $files_root.'/css/md2.css', '', '');
...
function mdx_comment_format($comment, $args, $depth) {
    ...
    <li class="mdui-list-item" id="li-comment-<?php comment_ID(); ?>">
        ...
        <div class="mdui-list-item-content outbu" id="comment-<?php comment_ID(); ?>">
            ...
            <span
                class="mdx-reply-time"><?php echo human_time_diff(get_comment_time('U'), current_time('timestamp')).__('前', 'mdx'); ?></span><?php comment_reply_link(array_merge($args, array('reply_text' => '<i class="mdui-icon material-icons">&#xe15e;</i> '.__('回复', 'mdx'), 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
        </div>
    </li>
...
function mdx_process_avatar($matches) {
    ...
    isset($img['class']) ? $mdx_img_class = $img['class']['value'].' ' : $mdx_img_class = '';
    isset($img['alt']) ? $mdx_img_alt = ' alt="'.$img['alt']['value'].'"' : $mdx_img_alt = '';
	return '<img'.$mdx_img_width.''.$mdx_img_height.' class="'.$mdx_img_class.'lazyload mdx-avatar-lazyload" src="'.$placeholder_image.'" data-src="'.$img['src']['value'].'"'.$mdx_img_alt.$mdx_srcset.''.$mdx_sizes.'>';
}
...
function mdx_shortcode_list_item($atts, $content = '') {
    extract(shortcode_atts(array(
        'action' => '',
        'blank' => 'false',
        'avatar' => '',
...
add_shortcode("mdx_list_item", "mdx_shortcode_list_item");
...
function mdx_add_plugin($plugin_array) {
    ...
    $plugin_array['mdx_list'] = get_bloginfo('template_url').'/js/sc1.js';
    return $plugin_array;
}
...
?>
    ...
    <select name="mdx_styles" id="mdx_styles">
        <option value="def"
                <?php if ($mdx_v_styles == 'def' || $mdx_v_styles == ''){ ?>selected="selected"<?php } ?>><?php _e('跟随全局设置', 'mdx'); ?></option>
        ...
        <option value="deep-purple" <?php if ($mdx_v_styles == 'deep-purple'){ ?>selected="selected"<?php } ?>>Deep
            Purple
        </option>
        ...
        <option value="light-blue" <?php if ($mdx_v_styles == 'light-blue'){ ?>selected="selected"<?php } ?>>Light
            Blue
        </option>
        ...
        <option value="light-green" <?php if ($mdx_v_styles == 'light-green'){ ?>selected="selected"<?php } ?>>Light
            Green
        </option>
        ...
        <option value="deep-orange" <?php if ($mdx_v_styles == 'deep-orange'){ ?>selected="selected"<?php } ?>>Deep
            Orange
        </option>
        ...
        <option value="blue-grey" <?php if ($mdx_v_styles == 'blue-grey'){ ?>selected="selected"<?php } ?>>Blue Grey
        </option>
    ...
    <select name="mdx_styles_act" id="mdx_styles_act">
        <option value="def"
                <?php if ($mdx_v_styles_act == 'def' || $mdx_v_styles_act == ''){ ?>selected="selected"<?php } ?>><?php _e('跟随全局设置', 'mdx'); ?></option>
        ...
        <option value="deep-purple" <?php if ($mdx_v_styles_act == 'deep-purple'){ ?>selected="selected"<?php } ?>>Deep
            Purple
        </option>
        ...
        <option value="light-blue" <?php if ($mdx_v_styles_act == 'light-blue'){ ?>selected="selected"<?php } ?>>Light
            Blue
        </option>
        ...
        <option value="light-green" <?php if ($mdx_v_styles_act == 'light-green'){ ?>selected="selected"<?php } ?>>Light
            Green
        </option>
        ...
        <option value="deep-orange" <?php if ($mdx_v_styles_act == 'deep-orange'){ ?>selected="selected"<?php } ?>>Deep
            Orange
        </option>
    </select>
    ...
    <select name="mdx_post_style" id="mdx_post_style">
        <option value="def"
                <?php if ($mdx_v_post_style == 'def' || $mdx_v_post_style == ''){ ?>selected="selected"<?php } ?>><?php _e('跟随全局设置', 'mdx'); ?></option>
        <option value="0"
                <?php if ($mdx_v_post_style == '0'){ ?>selected="selected"<?php } ?>><?php _e('标准', 'mdx'); ?></option>
        <option value="1"
                <?php if ($mdx_v_post_style == '1'){ ?>selected="selected"<?php } ?>><?php _e('简洁', 'mdx'); ?></option>
        <option value="2"
                <?php if ($mdx_v_post_style == '2'){ ?>selected="selected"<?php } ?>><?php _e('通透', 'mdx'); ?></option>
        <option value="3"
                <?php if ($mdx_v_post_style == '3'){ ?>selected="selected"<?php } ?>><?php _e('朴素', 'mdx'); ?></option>
    </select>
    ...
    <select name="mdx_post_show" id="mdx_post_show">
        <option value="0"
                <?php if ($mdx_v_post_show == '0' || $mdx_v_post_show == ''){ ?>selected="selected"<?php } ?>><?php _e('正常显示', 'mdx'); ?></option>
        <option value="1"
                <?php if ($mdx_v_post_show == '1'){ ?>selected="selected"<?php } ?>><?php _e('404模式', 'mdx'); ?></option>
        <option value="2"
                <?php if ($mdx_v_post_show == '2'){ ?>selected="selected"<?php } ?>><?php _e('隐藏模式', 'mdx'); ?></option>
        <option value="3"
                <?php if ($mdx_v_post_show == '3'){ ?>selected="selected"<?php } ?>><?php _e('对游客隐藏模式', 'mdx'); ?></option>
    </select>
